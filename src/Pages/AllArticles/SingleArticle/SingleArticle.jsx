...
const SingleArticle = ({ data, premiumArticle }) => {
    ...
    return (
        <div className={`relative ${isPremium ? 'bg-emerald-100' : 'bg-gray-100'} flex flex-col text-gray-700 rounded-md border-2 hover:shadow-xl  hover:duration-500 lg:w-[450px] container mx-auto bg-clip-border`}>
            ...
            <div className="p-6">
                <Link to={`/articleDetails/${_id}`}>
                    <button
                        className="group w-full rounded-md relative inline-block overflow-hidden border border-green-600 px-8 py-3 focus:outline-none focus:ring">
                        <span className="absolute inset-x-0 bottom-0 h-[2px] bg-green-600 transition-all group-hover:h-full group-green:bg-indigo-500"></span>
